...
-- main table with directory entries, if parent_id is NULL then this is the 
-- root, look for media_desc with root_id = this.id
CREATE TABLE dir_entry_desc(id INTEGER PRIMARY KEY, parent_id INTEGER, 
    ...
    desc_id INTEGER,
    FOREIGN KEY(parent_id) REFERENCES dir_entry_desc(id),  
    FOREIGN KEY(desc_id) REFERENCES text_desc(id));
...
CREATE TABLE tag_dir_entry_assoc (id INTEGER PRIMARY KEY, 
    tag_id INTEGER NOT NULL, dir_entry_id INTEGER NOT NULL, 
    FOREIGN KEY(tag_id) REFERENCES tag_desc(id),
    FOREIGN KEY(dir_entry_id) REFERENCES dir_entry_desc(id), 
    UNIQUE(tag_id, dir_entry_id));

-- indices to speed up search by tag_id or by dir_entry_id, e.g. we often 
